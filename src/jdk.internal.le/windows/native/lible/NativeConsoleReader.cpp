...
JNIEXPORT void JNICALL Java_jdk_internal_console_NativeConsoleReader_initIDs
  (JNIEnv *env, jclass) {
    ...
    KEY_EVENT_Class = (jclass) env->NewGlobalRef(env->FindClass("jdk/internal/console/NativeConsoleReader$KeyEvent"));
    ...
    WINDOW_SIZE_EVENT_Class = (jclass) env->NewGlobalRef(env->FindClass("jdk/internal/console/NativeConsoleReader$WindowSizeEvent"));
...
JNIEXPORT jint JNICALL Java_jdk_internal_console_NativeConsoleReader_cursorX
  (JNIEnv *env, jclass) {
    HANDLE h = GetStdHandle(STD_OUTPUT_HANDLE);
    CONSOLE_SCREEN_BUFFER_INFO buffer;
    if (!GetConsoleScreenBufferInfo(h, &buffer)) {
...
JNIEXPORT jobject JNICALL Java_jdk_internal_console_NativeConsoleReader_readEvent
  (JNIEnv *env, jclass) {
