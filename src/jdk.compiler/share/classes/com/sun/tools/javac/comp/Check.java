 ...
 **************************************************************************/
    ...
    long checkFlags(long flags, Symbol sym, JCTree tree) {
        ...
        case VAR:
            ...
            else if (sym instanceof RecordComponent)
                mask = VarFlags;
    ...
    private void validateAnnotation(JCAnnotation a, JCTree declarationTree, Symbol s) {
        ...
        if (a.type.tsym.isAnnotationType()) {
            ...
            if (!applicableTargetsOp.isEmpty()) {
                ...
                boolean isComponentWithNonApplicableDeclAnno = s instanceof RecordComponent && notApplicableOrIsTypeUseOnly;

                if (applicableTargets.isEmpty() || isCompRecordElementWithNonApplicableDeclAnno || isComponentWithNonApplicableDeclAnno) {
                    if (isCompRecordElementWithNonApplicableDeclAnno || isComponentWithNonApplicableDeclAnno) {
                         ...
                         * that are not applicable to the given record member from the Symbol:
                         */
    ...
    Optional<Set<Name>> getApplicableTargets(JCAnnotation a, Symbol s) {
        ...
        for (Name target : targets) {
            if (target == names.TYPE) {
                ...
                if (s.kind == VAR && s.owner.kind != MTH && s.getKind() != ElementKind.RECORD_COMPONENT)
            ...
            } else if (target == names.CLASS_COMPONENT) {
                if (s.getKind() == ElementKind.CLASS_COMPONENT) {
                    applicableTargets.add(names.CLASS_COMPONENT);
                }
