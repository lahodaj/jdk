...
public class SimpleRegressionTest extends KullaTesting {
    ...
    public void testMultiSnippetDependencies() {
        List<SnippetEvent> events = assertEval("int a = 3, b = a+a, c = b *100;",
                ...
                chain(added(VALID)),
                chain(added(VALID)));
        Assertions.assertEquals("3", events.get(0).value());
        Assertions.assertEquals("6", events.get(1).value());
        Assertions.assertEquals("600", events.get(2).value());
        assertEval("c;", "600");
    }
    ...
    public void testGenericMethodCrash() {
        ...
        Assertions.assertEquals("Integer", sne.typeName());
    }
    ...
    public void testLongRemoteStrings() { //8158906
        ...
        for (String len : new String[]{"12345", "64000", "65535", "65536", "120000"}) {
            ...
            Assertions.assertFalse(shut[0], "JShell died with long string");
            Assertions.assertEquals(1, el.size(), "Excepted one event");
            Assertions.assertTrue(el.get(0).value().length() > 10000,
