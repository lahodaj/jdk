...
public class RecordsTest extends KullaTesting {
    ...
    public void testRecordClass() {
        ...
        Assertions.assertEquals("r", varKey(assertEval("R r = new R(\"r\", 42);")).name());
        assertEval("r.s()", "\"r\"");
        assertEval("r.i()", "42");
    }
    ...
    public void testRecordField() {
        Assertions.assertEquals("record", varKey(assertEval("String record = \"\";")).name());
        assertEval("record.length()", "0");
    }

    @Test
    public void testRecordMethod() {
        Assertions.assertEquals("record", methodKey(assertEval("String record(String record) { return record + record; }")).name());
