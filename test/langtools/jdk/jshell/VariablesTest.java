...
//    }
    ...
    public void constantVariables() {
        var origConstantKey = varKey(assertEval("final int C1 = 0;"));
        assertEval("final int C2 = C1 + 1;");
        assertEval("""
                   switch (0) {
                       case C1 -> "C1";
                       case C2 -> "C2";
                   ...
                   """, "\"C1\"");
        var testMethod = methodKey(assertEval(
                   """
                   String testMethod(int input) {
                       return switch (input) {
                           case C1 -> "C1";
                           case C2 -> "C2";
                           default -> "other";
                   ...
                   """));
        assertEval("testMethod(-1)", "\"other\"");
        assertEval("testMethod(0)", "\"C1\"");
        assertEval("testMethod(1)", "\"C2\"");
        assertEval("testMethod(2)", "\"other\"");
        assertEval("testMethod(3)", "\"other\"");

        //constant re-declaration:
        var newConstantKey = varKey(assertEval("final int C1 = 1;",
                DiagCheck.DIAG_OK,
                DiagCheck.DIAG_ERROR,
                   ste(MAIN_SNIPPET, OVERWRITTEN, VALID, true, null),
                   ste(testMethod, VALID, RECOVERABLE_DEFINED, false, MAIN_SNIPPET),
                   ste(origConstantKey, VALID, OVERWRITTEN, false, MAIN_SNIPPET)));
        assertEval("testMethod(-1)", "\"other\"");
        assertEval("testMethod(0)", "\"other\"");
        assertEval("testMethod(1)", "\"C1\"");
        assertEval("testMethod(2)", "\"C2\"");
        assertEval("testMethod(3)", "\"other\"");

        //special cases:
        //a Unicode-encoded name:
        assertEval("final int C\\u0041 = 0;");
        //assigning to the constant:
        assertEval("C1 = 2");
    }
