...
public class InaccessibleExpressionTest extends KullaTesting {
    ...
    public void testExternal() {
        ...
        Assertions.assertEquals("priv.Packp", down.typeName());
        assertEval(down.name() + ".get()", "5");
        VarSnippet list = varKey(assertEval("list()", "[]"));
        Assertions.assertEquals("priv.MyList", list.typeName());
        assertEval(list.name() + ".size()", "0");
        VarSnippet one = varKey(assertEval("priv()", "One"));
        Assertions.assertEquals("priv.GetPriv.Count", one.typeName());
    ...
    public void testInternal() {
        ...
        Assertions.assertEquals("Top.Inner", n.typeName());
    }
