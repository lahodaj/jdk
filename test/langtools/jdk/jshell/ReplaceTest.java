...
public class ReplaceTest extends KullaTesting {
    ...
    private <T extends Snippet> void identityMatch(Stream<T> got, T expected) {
        ...
        Assertions.assertTrue(it.hasNext(), "expected exactly one");
        Assertions.assertTrue(expected == it.next(), "Identity must not change");
        Assertions.assertFalse(it.hasNext(), "expected exactly one");
    }
