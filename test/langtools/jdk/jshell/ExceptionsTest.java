...
public class ExceptionsTest extends KullaTesting {
    ...
    private void assertExceptionMatch(Throwable exception, String source, ExceptionInfo exceptionInfo) {
        Assertions.assertNotNull(exception, "Expected exception was not thrown: " + exceptionInfo.exception);
        if (exception instanceof EvalException) {
            ...
            if (exceptionInfo.message != null) {
                Assertions.assertEquals(exceptionInfo.message, ex.getMessage(),
            ...
            Assertions.fail("Unexpected exception: " + exception + " or exceptionInfo: " + exceptionInfo);
        }
    }
    ...
    private void assertAnyExceptionMatch(Throwable exception, AnyExceptionInfo exceptionInfo) {
        if (exceptionInfo instanceof ExceptionInfo) {
            ...
            Assertions.assertTrue(exceptionInfo instanceof UnresolvedExceptionInfo, "Bad exceptionInfo: " + exceptionInfo);
            Assertions.assertTrue(exception instanceof UnresolvedReferenceException,
            ...
            Assertions.assertEquals(uei.sn, ure.getSnippet());
    ...
    private void assertStackTrace(StackTraceElement[] actual, StackTraceElement[] expected, String message) {
        if (actual != expected) {
            if (actual == null || expected == null) {
                ...
                Assertions.assertEquals(expected.length, actual.length, message + " : arrays do not have the same size");
                for (int i = 0; i < actual.length; ++i) {
                    StackTraceElement actualElement = actual[i];
                    StackTraceElement expectedElement = expected[i];
                    Assertions.assertEquals(expectedElement.getClassName(), actualElement.getClassName(), message + " : class names [" + i + "]");
                    String expectedMethodName = expectedElement.getMethodName();
                    if (expectedMethodName.startsWith("lambda$")) {
                        Assertions.assertTrue(actualElement.getMethodName().startsWith("lambda$"), message + " : method names");
                    } else {
                        Assertions.assertEquals(expectedElement.getMethodName(), actualElement.getMethodName(), message + " : method names [" + i + "]");
                    }
                    Assertions.assertEquals(expectedElement.getFileName(), actualElement.getFileName(), message + " : file names [" + i + "]");
                    if (expectedElement.getLineNumber() >= 0) {
                        Assertions.assertEquals(expectedElement.getLineNumber(), actualElement.getLineNumber(), message + " : line numbers [" + i + "]"
