...
public class ErrorTranslationTest extends ReplToolTesting {
    ...
    private Consumer<String> assertDiagnostic(String expectedSource, ExpectedDiagnostic expectedDiagnostic) {
        ...
        return s -> {
            ...
            Assertions.assertEquals(kind, lines[0]);
            boolean found = false;
            for (int i = 0; i < lines.length; i++) {
                if (lines[i].endsWith(expectedSource)) {
                    Assertions.assertEquals(expectedMarkingLine, lines[i + 1], "Input: " + expectedSource + ", marking line: ");
                    found = true;
                }
            }
            if (!found) {
    ...
    private String createMarkingLine(int start, int end) {
        Assertions.assertTrue(end >= start, String.format("End position %d is less than start position %d", end, start));
