...
public class FlagsTest {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        findFreeFlags();
        verifyAllExtendedStandardFlagsAreTreeFlags();
    }
    ...
    private static void findFreeFlags() throws Throwable {
        ...
        long freeTreeFlags = ~collectFlags(target2Flag2Fields, FlagTarget.TREE, FlagTarget.TYPE, FlagTarget.METHOD, FlagTarget.VARIABLE);
        long freeTypeFlags = ~collectFlags(target2Flag2Fields, FlagTarget.TREE, FlagTarget.TYPE);
        long freeMethodFlags = ~collectFlags(target2Flag2Fields, FlagTarget.TREE, FlagTarget.METHOD);
        long freeVariableFlags = ~collectFlags(target2Flag2Fields, FlagTarget.TREE, FlagTarget.VARIABLE);

        printFreeFlags("TREE", freeTreeFlags);
        printFreeFlags("TYPE", freeTypeFlags);
        printFreeFlags("METHOD", freeMethodFlags);
        printFreeFlags("VARIABLE", freeVariableFlags);
    }

    private static void verifyAllExtendedStandardFlagsAreTreeFlags() throws Throwable {
        Map<FlagTarget, Map<Long, List<Field>>> target2Flag2Fields = computeTarget2Flag2Fields();

        long treeFlags = collectFlags(target2Flag2Fields, FlagTarget.TREE);

        if ((Flags.ExtendedStandardFlags & ~treeFlags) != 0) {
            throw new AssertionError("Not all ExtendedStandardFlags are TREE flags: " +
                                     Flags.asFlagSet(Flags.ExtendedStandardFlags & ~treeFlags));
        }
    }
    ...
    private static void printFreeFlags(String comment, long freeFlags) {
            ...
            for (int bit = 16; bit < 64; bit++) { //lower 16 bits are used in classfiles, never suggest adding anything there
